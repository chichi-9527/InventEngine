...
namespace INVENT
{
	...
	IVulkanTexture2DManagement::Texture2DHandle IVulkanTexture2DManagement::AddTexture2D(const std::string& name, const std::string& path, TextureType texture_type)
	{
		...
		IEngineTools::Instance().GetWorkThreadPool()->Submit(0, [this, handle, &path, texture_type]() {
			...
			if (!VulkanBase::Base().UseVmaCreateBuffer(imageSize,
			...
			void* data;
			VulkanBase::Base().UseVmaMapMemory(stagingBuffer, data);
			memcpy(data, texData, static_cast<size_t>(imageSize));
			VulkanBase::Base().UseVmaUnmapMemory(stagingBuffer);
			...
			VulkanBase::Base().UseVmaCreateImage(width,
				height,
				levelCount,
				textureFormat,
			...
			VulkanBase::Base().UseVmaDestroyBuffer(stagingBuffer);

			VkImageView imageView = VulkanBase::Base().CreateImageView(image,
				textureFormat,
			...
			VulkanBase::Base().UpdateBindlessTextureSlot(static_cast<uint32_t>(handle.GetRealIndex()), imageView);
			_textures[handle.GetRealIndex()] = { image,imageView };
			_bit_vector_vaild.SetValue<true>(handle);
			});
	...
	IVulkanTexture2DManagement::Texture2DHandle IVulkanTexture2DManagement::AddTexture2D(const std::string& name, VkImage image, VkImageView image_view)
	{
		...
		IEngineTools::Instance().GetWorkThreadPool()->Submit(0, [this, handle, image, image_view]() {
			VulkanBase::Base().UpdateBindlessTextureSlot(static_cast<uint32_t>(handle.GetRealIndex()), image_view);
			_textures[handle.GetRealIndex()] = { image,image_view };
			_bit_vector_vaild.SetValue<true>(handle);
			});
	...
	void IVulkanTexture2DManagement::_init_default_image()
	{
		...
		VkBuffer stagingBuffer;
		if (!VulkanBase::Base().UseVmaCreateBuffer(stagingBufferSize,
			VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
			VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
			stagingBuffer))
		{
		...
		// white
		VkImage whiteImage;
		VulkanBase::Base().UseVmaCreateImage(1,
			1,
			1,
		...
		_upload_texture_and_generate_mipmaps(stagingBuffer,
			...
			1,
			0);
		VkImageView whiteImageView = VulkanBase::Base().CreateImageView(whiteImage,
			VK_FORMAT_R8G8B8A8_SRGB,
			VK_IMAGE_ASPECT_COLOR_BIT,
			VK_IMAGE_VIEW_TYPE_2D,
			1);
		...
		VulkanBase::Base().UpdateBindlessTextureSlot(0, whiteImageView);
		_textures[0] = { whiteImage,whiteImageView };
		_bit_vector_vaild.SetValue<true>(IHandle(0));
		...
		VkImageView blackImageView = VulkanBase::Base().CreateImageView(blackImage,
			VK_FORMAT_R8G8B8A8_SRGB,
			VK_IMAGE_ASPECT_COLOR_BIT,
			VK_IMAGE_VIEW_TYPE_2D,
			1);
		...
		VulkanBase::Base().UpdateBindlessTextureSlot(1, blackImageView);
		_textures[1] = { blackImage,blackImageView };
		_bit_vector_vaild.SetValue<true>(IHandle(1));
		...
		VkImageView normalImageView = VulkanBase::Base().CreateImageView(normalImage,
			VK_FORMAT_R8G8B8A8_UNORM,
			VK_IMAGE_ASPECT_COLOR_BIT,
			VK_IMAGE_VIEW_TYPE_2D,
			1);
		...
		VulkanBase::Base().UpdateBindlessTextureSlot(2, normalImageView);
		_textures[2] = { normalImage,normalImageView };
		_bit_vector_vaild.SetValue<true>(IHandle(2));
	...
	void IVulkanTexture2DManagement::_transition_image_layout(VkCommandBuffer commond_buffer, VkImage image, VkFormat format, VkImageLayout old_layout, VkImageLayout new_layout, uint32_t base_mip_level, uint32_t level_count)
	{
		...
		else
		{
			...
			barrier.image = image;
			barrier.subresourceRange = { VK_IMAGE_ASPECT_COLOR_BIT, base_mip_level, level_count, 0, 1 };
			barrier.srcAccessMask = srcAccess;
			barrier.dstAccessMask = dstAccess;

			vkCmdPipelineBarrier(commond_buffer, srcStage, dstStage, 0, 0, nullptr, 0, nullptr, 1, &barrier);
		}
	}
	...
	{
		auto cmd = VulkanBase::Base().BeginSingleTimeCommands();
		...
		_transition_image_layout(cmd, 
			...
			level_count - 1,
			1);
		VulkanBase::Base().EndSingleTimeCommands(cmd);
	}

	const IHandle& IVulkanTexture2DManagement::_find_handle_from_cache(const std::string& name) const
	{
